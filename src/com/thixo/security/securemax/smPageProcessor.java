...
public class smPageProcessor {
    ...
    private smBase64 Base64;

    public smPageProcessor() {
	this.Threader = new smThreader();
	this.CRC32 = new smCRC32();
	this.HexTrig = new smHexTrig();
	this.Base64 = new smBase64();
    }

    public static void main(String args[]) {
	smPageProcessor pTest = new smPageProcessor();
	String encData = new String(pTest.encodeParams("test.me.com", "\"orderaddupdate.cfm?szParam1=Values1&szParam2=Values2\""));
	String decData = new String(pTest.DecodeParams("test.me.comorderaddupdate.cfm", encData.substring(0, encData.length() - 1)));
	...
	while(curIndex < inData.length()) {
	    idx = (inData.indexOf("/", curIndex) == inData.lastIndexOf("/") ? inData.length() : inData.indexOf("/", curIndex + 1));
	    ...
	    if(!enCRC32.equals(threadCRC)) return(new String(""));
	    threadData = this.Threader.dethread(enData);
	    enServerPage = new String(threadData[0]);
	    if(!enServerPage.equalsIgnoreCase(serverPage)) return(new String(""));
	    enParams = new String(threadData[1]);
	    outData += enParams;
	    if(curIndex < inData.length()) outData += "&";
	}
	return(outData);
    }
}
